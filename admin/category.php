...
if (isset($_POST['addContent'])) {
    ...
    if ($count == 0) {
        ...
        if ($iconError === UPLOAD_ERR_OK) {
            // Check the file size
            if ($iconSize <= 1048576) { // 1 MB (1 MB = 1048576 bytes)
                $iconDestination = 'icons/' . $iconName;
                
                // Move the uploaded file to the destination directory
                if (move_uploaded_file($iconTmpName, $iconDestination)) {
                    // Insert the data into the database
                    $sql = "INSERT INTO `category` (`cname`, `icon`) VALUES ('$cname', '$iconName')";
            ...
            echo "Error uploading the file: " . $iconError;
        }
    } else {
