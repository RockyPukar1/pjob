...
// Get the c_id from the query parameter if provided
$c_id = isset($_GET['c_id']) ? intval($_GET['c_id']) : 0; // Make sure it's an integer

// Build the SQL query
$sql = "SELECT j.*, c.c_logo FROM jobs j
        INNER JOIN company c ON j.c_id = c.c_id";

// If a valid c_id is provided, add a WHERE clause to filter jobs
if ($c_id > 0) {
    $sql .= " WHERE j.c_id = $c_id";
}
